...
Each challenge is self-contained and builds on previous ones.
...
### Challenge 1.2 — Multi-Service with Docker Compose

**Goal**: Connect multiple containers together.

Currently `docker-compose.yml` has only one service (`web`). Add a second service:
...
1. Stage 1: Use a Node.js image to build a simple static site (or use any build tool you like).
